...
const SignupForm: React.FC<SignupFormProps> = ({ defaultRole }) => {
  ...
  const validateForm = useCallback((): FormErrors => {
    ...
    if (defaultRole === "technician") {
      ...
      if (!formData.authorizedPhone1.match(/^\d{10}$/))
      ...
      if (!formData.authorizedPhone2.match(/^\d{10}$/))
  ...
  const handleChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
      ...
      if (
        (name === "mobile" || name === "authorizedPhone1" || name === "authorizedPhone2") &&
        value &&
        !/^\d*$/.test(value)
      ) {
        setErrors((prev) => ({
            ...
            <>
              ...
              <div>
                ...
                {errors.subscriptionId && (
                  <div className="text-red-500 text-xs mt-1">{errors.subscriptionId}</div>
                )}
              </div>
            </>
          )}
