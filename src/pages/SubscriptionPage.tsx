...
import { BadgeIndianRupee, LucideIcon } from 'lucide-react';
import { plans } from '../data/subscriptionPlans';
import PlanDetails from './PlanDetailsPage';
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 xl:gap-10">
            {plans.map((plan: Plan) => {
              ...
              return (
                <div
                  key={plan.id}
                  className={`relative bg-white rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:scale-105 border ${plan.popular ? 'border-yellow-400 ring-2 ring-yellow-400' : 'border-gray-200'}`}
                >
        ...
        {selectedPlan && (
          <div className="mt-12">
            <button
              ...
              className="mb-6 px-4 py-2 text-sm font-medium text-blue-600 bg-blue-100 rounded-lg hover:bg-blue-200 transition"
            >
              ← Back to All Plans
            </button>
