...
const TechnicianSubscriptionPlans = () => {
  ...
  // Fetch technician-specific remaining leads
  const fetchTechnicianLeads = async () => {
    try {
      ...
      const response = await getTechnicianLeads(techId);
      if (response?.success) {
        setTechnicianLeads(response.data || {});
      } else {
        console.error('Failed to fetch technician leads:', response?.message);
      }
    } catch (err) {
      console.error('Error fetching technician leads:', err);
    }
  };
